/**
 * Financial Engine for Proyectos 4L
 * Handles all projections, occupancy curves, and inflation math.
 */

const memo = new Map();

/**
 * Calculates a 10-year financial projection.
 * Results are memoized based on input variables.
 */
export function calculateYearlyProjection(variables, tickets) {
    // Simple key for memoization
    const key = JSON.stringify({ variables, tickets });
    ...
    memo.set(key, yearlyData);
    return yearlyData;
}

/**
 * Calculates recovery time for a specific ticket price
 */
export function calculateRecovery(yearlyData, precio, anios) {
    ...
    for (let yr = 0; yr < anios; yr++) {
        ...
        if (runningSumRecup + u >= precio) {
            ...
            return (yr + fraction).toFixed(1) + ' años';
        }
        runningSumRecup += u;
    }
    return '10+ años';
